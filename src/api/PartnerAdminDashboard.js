...
import { getFunctions } from 'backend/functions';
...
const PartnerAdminDashboard = () => {
  ...
  useEffect(() => {
    ...
    if (['admin', 'institutionAdmin'].includes(currentUserRole) && !instIdForSession) {
  ...
  const handleSubmitCaregiverPasswordReset = async (event) => {
    ...
    try {
      ...
      // Send a password reset email to the caregiver instead.
      const { sendPasswordResetEmail } = await import('backend/auth');
      const { auth } = await import('../backend/config');
      await sendPasswordResetEmail(auth, caregiverForPasswordReset.email);
...
if (['admin', 'institutionAdmin'].includes(currentUserRole) && !finalPartnerId && !loading) {
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
        ...
        <aside className="fixed inset-y-0 left-0 w-72 max-w-[85vw] bg-white border-r border-gray-200 flex flex-col h-screen shadow-xl">
          <div className="p-4 border-b border-gray-200 flex items-start justify-between gap-3">
            <div className="min-w-0">
              <img src="/images/caremaster-logo.jpg" alt="Caremaster" className="h-8 mb-2" />
              <p className="text-xs font-semibold uppercase tracking-[0.3em] text-blue-600">Partner admin</p>
              <h1 className="text-sm font-semibold text-gray-900 truncate">{institutionData?.name || 'Partner'}</h1>
            </div>
            <button
        ...
        <div className="max-w-6xl mx-auto space-y-6">
          <header className="flex items-center justify-between">
            <div className="flex flex-col gap-2">
              ...
              <h1 className="text-3xl font-semibold text-gray-900">{institutionData?.name || 'Partner Dashboard'}</h1>
