  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        console.warn('⚠️ Database failed in PartnerAdminGuard, trying localStorage cache:', databaseError.message);
      }

      // Fallback: if Database failed or doc missing, try localStorage cache
      if (!userProfile) {
        try {
