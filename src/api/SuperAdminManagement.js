...
import { collection, query, getDocs, deleteDoc, where, doc } from 'backend/database';
...
const SuperAdminManagement = () => {
  ...
  const handleResetPassword = async () => {
    ...
    try {
      ...
      setMessage(`Password reset email sent to ${selectedAdmin.email}`);
      ...
      setMessage(`Failed to send password reset: ${error.message}`);
    } finally {
  ...
  const handleDeleteAdmin = async () => {
    ...
    try {
      // Delete Database user document (Auth record requires Admin SDK / backend)
      await deleteDoc(doc(db, 'users', selectedAdmin.id));
      setMessage(`Super admin ${selectedAdmin.email} removed from database. Note: Backend Auth record must be deleted separately via console.`);
      setShowDeleteConfirm(false);
      setSelectedAdmin(null);
