...
const InstitutionCaregiverGuard = ({ children }) => {
  ...
  useEffect(() => {
    const checkAccess = async () => {
      ...
      // Check if onboarding is required — mandatory for ALL caregivers regardless of status
  ...
  if (!loading && userProfile) {
    ...
    // CRITICAL: Redirect to onboarding if incomplete — mandatory for ALL caregivers
    if (!userProfile.onboardingComplete) {
