...
const PartnerLinkCustomizer = ({ institution, onUpdate, onClose }) => {
  ...
  const generatePreviewUrls = () => {
    ...
    if (domain) {
      ...
      return {
        landing: `${baseUrl}/login?institution=${institution.id}`,
        admin: `${baseUrl}/login?institution=${institution.id}&role=admin`,
        caregiver: `${baseUrl}/login?institution=${institution.id}&role=caregiver`,
        pharmacist: `${baseUrl}/login?institution=${institution.id}&role=pharmacist`
      };
    }
  };
