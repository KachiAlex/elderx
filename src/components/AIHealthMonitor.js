...
const AIHealthMonitor = ({ isOpen, onClose, clientData }) => {
  ...
  }, [isOpen, clientData, autoAnalysis]);
  ...
  const analyzeVitalSigns = async (vitalSignsData) => {
    ...
    try {
      const analysis = await aiService.analyzeVitalSigns(vitalSignsData, clientData);
