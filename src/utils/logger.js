...
class Logger {
  ...
  persistLog(logEntry) {
    try {
      const existingLogs = JSON.parse(localStorage.getItem('UltimateCare_logs') || '[]');
      ...
      localStorage.setItem('UltimateCare_logs', JSON.stringify(existingLogs));
    } catch (e) {
  ...
  getCurrentUserId() {
    try {
      ...
      const user = JSON.parse(localStorage.getItem('UltimateCare_user') || '{}');
  ...
  clearLogs() {
    this.logs = [];
    localStorage.removeItem('UltimateCare_logs');
  }
