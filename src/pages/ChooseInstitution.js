...
const ChooseInstitution = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const results = await Promise.all(ids.map(id => institutionAPI.getInstitution(id)));
        setInstitutions(results.filter(Boolean));
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      <h1 className="text-xl font-semibold mb-4">Select an Institution</h1>
