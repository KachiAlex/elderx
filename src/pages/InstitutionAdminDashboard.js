  ...
  const loadDashboardData = async () => {
    try {
      ...
      const realStats = {
        ...
        patients: institutionUsers.filter(u => u.userType === 'elderly' || u.userType === 'client' || u.userType === 'patient').length,
        caregivers: institutionUsers.filter(u => u.userType === 'caregiver').length,
        doctors: institutionUsers.filter(u => u.userType === 'doctor').length,
        nurses: institutionUsers.filter(u => u.userType === 'nurse').length,
  ...
  const quickActions = [
    {
      ...
      action: () => navigate('/institution-admin/users?action=create')
    },
    {
      name: 'View Analytics',
      icon: BarChart3,
    ...
    },
    {
      ...
      action: () => navigate('/institution-admin/settings')
    }
  ];
