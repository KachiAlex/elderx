...
const PartnerCaregiverOnboarding = () => {
  ...
  useEffect(() => {
    ...
    if (userProfile?.onboardingComplete) {
      navigate(`/institution-caregiver/dashboard${effectivePartnerId ? `?institution=${effectivePartnerId}` : ''}`);
    }
  }, [userProfile, navigate, effectivePartnerId]);
  ...
  const handleStep2Upload = async () => {
    ...
    try {
      ...
      // Upload all documents and collect URLs
      const documentUrls = {};
        ...
        if (file) {
          ...
          uploadPromises.push(
            uploadCaregiverDocument(user.uid, file, `documents/${docType}`)
              .then((result) => {
                documentUrls[docType] = result.url;
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (userType === 'pharmacist') {
          window.location.href = `/institution-pharmacy/dashboard${effectivePartnerId ? `?institution=${effectivePartnerId}` : ''}`;
        } else {
          window.location.href = `/institution-caregiver/dashboard${effectivePartnerId ? `?institution=${effectivePartnerId}` : ''}`;
        }
      }, 1500);
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-6">
          <button
            onClick={() => navigate(`/login?institution=${effectivePartnerId}`)}
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            Welcome to {institutionData?.name || 'Partner'} Portal
          </h1>
...
export default PartnerCaregiverOnboarding;
