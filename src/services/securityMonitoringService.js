...
class SecurityMonitoringService {
  ...
  storeEvent(event) {
    try {
      ...
      localStorage.setItem('UltimateCare_security_events', JSON.stringify(events));
    } catch (error) {
  ...
  getStoredEvents() {
    try {
      const stored = localStorage.getItem('UltimateCare_security_events');
  ...
  getSessionId() {
    return sessionStorage.getItem('UltimateCare_session_id') || 'unknown';
  }

  getLastActivityTime() {
    return parseInt(localStorage.getItem('UltimateCare_last_activity') || Date.now());
  }
